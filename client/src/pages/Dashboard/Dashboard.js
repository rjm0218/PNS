...
function Dashboard() {
	...
	const [buildGearOptions, setBuildOptions] = useState(['Builder Hammer','Builder Shield','Builder Jacket', 'Builder Helmet','Builder Ring','Builder Boots']);
	const [gearLevelsBuild, setGearLevelsBuild] = useState([]);
	const [isCheckedBuild, setChecksBuild] = useState(null);
	const [resGearOptions, setResOptions] = useState(['Researcher Pincers','Researcher Shield','Researcher Coat', 'Researcher Mask','Researcher Ring','Researcher Boots']);
	const [gearLevelsRes, setGearLevelsRes] = useState([]);
	const [isCheckedRes, setChecksRes] = useState(null);
	const [heroOptions, setHeroOptions] = useState(['Chester','Anahita','Kento','Xavis']);
		...
		try {
		  ...
		  if (response.status === 200) {
			  ...
			  if (accs.length > 0) {
				...
				let newBuildGearList = Array.from({ length: accs.length }, () => Array(buildGearOptions.length).fill(0));
				let newBuildCheckedList = Array.from({ length: accs.length }, () => Array(buildGearOptions.length).fill(false));
				let newResGearList = Array.from({ length: accs.length }, () => Array(resGearOptions.length).fill(0));
				let newResCheckedList = Array.from({ length: accs.length }, () => Array(resGearOptions.length).fill(false));
				let newHeroList = Array.from({ length: accs.length }, () => Array.from({ length: heroOptions.length}, () => new Array(2).fill(0)));
				let newHeroCheckedList = Array.from({ length: accs.length }, () => Array(heroOptions.length).fill(false));
				let newBoosts = Array.from({ length: accs.length }, () => Array(2).fill(0));
				
				for (let i = 0; i < accs.length; i++) {
					let speeds = accs[i].boosts.speeds;
					if (speeds && speeds.length > 0) {
						newBoosts[i][0] = accs[i].boosts.speeds.find(boost => boost.name === 'Build Speed').value;
						newBoosts[i][1] = accs[i].boosts.speeds.find(boost => boost.name === 'Research Speed').value;
					}
	...
	const addAccount = async () => {
		if (accName) {
			try {
			  ...
			  console.error('Account creation failed:', error.response ? error.response.data : 'No response');
			}
		}
	};
	...
	const handleGearChange = async (gearLevel, gearindex, acToUpdate, type) => {
		...
		let newGear = JSON.stringify({ 'name': gearPiece, 'level': gearLevel, 'index': gearindex});
		
		try {
			const response = await api.post('/updateGearLevel', { user, acToUpdate, newGear,type  });
			if (response == null) {
	...
	const deleteGear = async (accindex, gearindex, type) => {
		...
		try {
			const response = await api.post('/deleteGear', { user, acToUpdate, gearPiece, type  });
			if (response == null) {
				console.log('Failed to set new gear level for ', accName);
			} else {
				getUserData();
	...
	const handleHeroChange = async (accindex, heroindex, newPlus, newLevel) => {
		...
		let acToUpdate = accounts[accindex].name;
		let newHero = JSON.stringify({ 'name': hero, 'plus': newPlus, 'level': newLevel});

		try {
			const response = await api.post('/updateHeroLevel', { user, acToUpdate, newHero  });
			if (response == null) {
				console.log('Failed to set new gear level for ', accName);
			} else {
				getUserData();
	...
	const deleteHero = async (accindex, index) => {
		...
		try {
			const response = await api.post('/deleteHero', { user, acToUpdate, hero  });
			if (response == null) {
				console.log('Failed to set new gear level for ', accName);
			} else {
				getUserData();
	...
	const handleChange = async (event, accindex, boostname) => {
		...
		const updateBoosts = async(accindex, boostname, boostvalue) => {
			...
			try {
				const response = await api.post('/updateBoosts', { user, acToUpdate, newboost  });
				if (response == null) {
    ...
    <div className="dashboard-container">
		...
		<div className="dashboard-content">
			...
			<div className="profile-sections">
				<Row className="mb-3">
					<Col>
						<InputGroup size={viewSize} className="acc-creation">
							<InputGroup.Text id="inputGroup-sizing-sm">Account Name</InputGroup.Text>
					...
					<Col xs={8}>
						<Button variant="secondary" size={viewSize} className="add-button" onClick={addAccount}>Create Account</Button>
					</Col>
				</Row>
				<Row className="mb-3">
					<Col xs={5}>
				...
				<Accordion>
					{isInitialized && accounts.map((acc, accindex) => (
						<Accordion.Item key={'item'+accindex} eventKey={accindex} className='account-info'>
							<Accordion.Header key={'header'+accindex} className='acc-header'>
								<h6>{acc.name + " - HQ " + acc.buildings.find(building => building.name === "HQ").level}</h6>
							</Accordion.Header>
							<Accordion.Body key={'body'+accindex} className='acc-body'>
								<Row>
									...
									<Col>
										<Button key={'button'+accindex} variant="danger" onClick={() => handleDelete(acc.name)}>Delete Account</Button>
									</Col>
								</Row>
								<Row>
									<Col>
											...
											<Row key={opt}>
												<Col xs='auto'>
													<Form.Check
														...
														onChange={() => handleCheck(accindex,heroindex,'hero')}
													/>
												</Col>
												<Col xs={6}>
													<Row>
														{isCheckedHeros[accindex][heroindex] &&
															<Col>
																<Form.Select className='hero-plus-level-select' 
																	size='sm' 
																	defaultValue={heroLevels[accindex][heroindex][0]}
																	onChange={(ev) => handleHeroChange(accindex, heroindex, ev.target.value, heroLevels[accindex][heroindex][1])}
																>
																	{Array.from({ length: 6 }, (_, index) => index).map(level => (
														...
														{isCheckedHeros[accindex][heroindex] &&	
															<Col>
																<Form.Select className='hero-level-select' 
																	size='sm' 
																	defaultValue={heroLevels[accindex][heroindex][1]}
																	onChange={(ev) => handleHeroChange(accindex, heroindex, heroLevels[accindex][heroindex][0], ev.target.value)}
																>
											...
											<Row>
												...
												<Col xs={3}>
													<Form.Control size='sm' type="text"
														...
														onChange={(ev) => handleChange(ev, accindex, 'Build Speed')}														
													/>
												</Col>
											</Row>
											...
											<Row key={opt}>
												<Col>
													<Form.Check
														...
														onChange={() => handleCheck(accindex,gearindex,'build')}
													/>
												</Col>
												<Col>

												{isCheckedBuild[accindex][gearindex] &&
													<Form.Select className='gear-level-select' size='sm' defaultValue={gearLevelsBuild[accindex][gearindex]} onChange={(evt) => handleGearChange(evt.target.value, gearindex, acc.name, 'build')}>
														{Array.from({ length: 5 }, (_, index) => index + 3).map(level => (
															<option key={level} value={level}>{level + '*'}</option>
														))}
											...
											<Row>
												...
												<Col xs={3}>
													<Form.Control size='sm' type="text"
														...
														onChange={(ev) => handleChange(ev, accindex, 'Research Speed')}														
													/>
												</Col>
											</Row>
											...
											<Form.Group as={Row} key={opt}>
												<Col>
													<Form.Check
														...
														onChange={() => handleCheck(accindex,gearindex,'research')}
													/>
												</Col>
												<Col>
												{isCheckedRes[accindex][gearindex] &&
													<Form.Select className='gear-level-select' size='sm' defaultValue={gearLevelsRes[accindex][gearindex]} onChange={(evt) => handleGearChange(evt.target.value, gearindex, acc.name,'research')}>
														{Array.from({ length: 5 }, (_, index) => index + 3).map(level => (
															<option key={level} value={level}>{level + '*'}</option>
														))}
